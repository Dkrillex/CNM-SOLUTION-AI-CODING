...
const SYSTEM = `You are cnmsoltion.ai Builder. Generate a REAL, runnable Next.js 16 App Router product from the user's brief.
...
<p class="font-mono text-xs uppercase tracking-widest text-lime-400">cnmsoltion.ai · generated</p>
...
export function appFromAiJson(
  ...
  const name = String(data.name || "cnmsoltion.ai App").slice(0, 80);
  const slug =
...
const EDIT_SYSTEM = `You are cnmsoltion.ai Builder. Revise an existing generated project.
