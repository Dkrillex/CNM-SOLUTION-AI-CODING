...
export function generateApp(prompt: string): GeneratedApp {
  ...
  const seed = titleCase(clean.replace(/^(a|an|the)\s+/i, "")).slice(0, 42) || "cnmsoltion App";
  ...
  return {
    ...
    schema: meta.tables,
    files,
    previewHtml: `<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${name}</title><script src="https://cdn.tailwindcss.com"></script></head><body class="min-h-screen bg-zinc-950 text-zinc-50"><main class="mx-auto max-w-3xl px-6 py-20"><p class="font-mono text-xs uppercase tracking-widest text-lime-400">cnmsoltion.ai · ${kind}</p><h1 class="mt-4 text-5xl font-semibold">${name}</h1><p class="mt-4 text-zinc-400">${clean}</p></main></body></html>`,
